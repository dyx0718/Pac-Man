...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            #   dx, dy = Actions.directionToVector(action)
            #   nextx, nexty = int(x + dx), int(y + dy)
            #   hitsWall = self.walls[nextx][nexty]c
            ...
            if not hitsWall:
                ...
                successor = ((next, successorVisited), action, 1)
...
def helper(position, nonVisitedCorners):
    closetCorner = nonVisitedCorners.pop()
    for corner in nonVisitedCorners:
        if util.manhattanDistance(position, corner) < util.manhattanDistance(position, closetCorner):
            closetCorner = corner
    return closetCorner
...
def foodHeuristic(state, problem):
    ...
    heuristic = 0
    for i in lsFoodGrid:
        tmp = util.manhattanDistance(position,i)
        if tmp > heuristic:
            heuristic = tmp
